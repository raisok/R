kmeans_plot<-function(filename="",groupname="",mincenter=3,maxcenter=5,sortway=1,plotway="all",writelist=T,cellWidth=8, cellHeight=.01){
  ...
  data<-read.delim(filename,row.names=1,check.names=F)
      ...
      n<-n}else{
        ...
        for (a in 1:ncenter){
          for (b in 1:sample_num){
            weightlist[a,b]<-sum(colMeans(mean_all_cluster[[a]][1:sample_num])/d[(sample_num-b+1):(2*sample_num-b)])}   #求权重
        }		
