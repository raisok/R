...
#args[5] = "Sham_vs_12-HETE.fpkm.txt"
#args[6] = "Sham_vs_12-HETE.biological_process_enrichment.xls"
#args[7] = "Sham"
#args[8] = "12-HETE"


fpkm <- read.csv(args[5],header=T,sep="\t",check.names=F)
GO_list <- read.csv(args[6],header=T,sep="\t",check.names=F)
if(nrow(GO_list)>=50){
GO_list <- GO_list[1:50,]
}else if (nrow(GO_list)==0){
...
name <- as.character(GO_list[i,]$Description)
name1 <- gsub("/|'|\"","",name)
name1 <- gsub(" ","_",name1)
